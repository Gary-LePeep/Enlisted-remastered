const TANKS_META = [
  {
    'Category': 'weapons.category.moscow.allies',
    Tanks: [
      'ussr_ba_11',
      'ussr_bm_8_24',
      'ussr_bt_7_1937',
      'us_m3_lee',
      'ussr_t_26_1940',
      'ussr_t_28',
      'ussr_t_28e',
      'ussr_t_34_1940_l_11',
      'ussr_t_34_1941',
      'ussr_t_60_1941',
      'ussr_t_50',
      'ussr_valentine_i'
    ]
  },
  {
    'Category': 'weapons.category.moscow.axis',
    Tanks: [
      'germ_pzkpfw_38t_ausf_f',
      'germ_pzkpfw_ii_ausf_c',
      'germ_pzkpfw_iii_ausf_b',
      'germ_pzkpfw_iii_ausf_e',
      'germ_pzkpfw_iii_ausf_j',
      'germ_pzkpfw_iii_ausf_j1_l60',
      'germ_pzkpfw_iv_ausf_e',
      'germ_pzkpfw_iv_ausf_f',
      'germ_pzkpfw_iv_ausf_f2'
    ]
  },
  {
    'Category': 'weapons.category.normandy.allies',
    Tanks: [
      'us_m4_calliope',
      'churchill_3',
      'us_m4a1_76w_sherman',
      'us_m4a2_sherman_1944',
      'us_m4a3e2_sherman_jumbo',
      'us_m5a1_stuart_late',
      'us_m8_scott',
      'us_m10_wolverine',
      'us_m18_hellcat',
      'us_m24_chaffee',
      'uk_sherman_ic_firefly'
    ]
  },
  {
    'Category': 'weapons.category.normandy.axis',
    Tanks: [
      'germ_panzerwerfer_42',
      'germ_pzkpfw_v_ausf_a_panther',
      'germ_panzerjager_iv_l_70',
      'germ_pzkpfw_iii_ausf_m',
      'germ_pzkpfw_iii_ausf_n',
      'germ_pzkpfw_iv_ausf_h',
      'germ_pzkpfw_iv_ausf_j',
      'germ_sdkfz_234_2',
      'germ_stug_iii_ausf_g',
      'germ_pzkpfw_vi_ausf_e_tiger'
    ],
  },
  {
    'Category': 'weapons.category.berlin.allies',
    Tanks: [
      'ussr_is_1',
      'ussr_is_2_1943',
      'us_m4a2_76w_sherman',
      'ussr_su_85m',
      'ussr_su_100_1945',
      'ussr_t_34_85_zis_53',
      'ussr_t_34_85_d_5t',
      'ussr_t_34_85_e'
    ]
  },
  {
    'Category': 'weapons.category.berlin.axis',
    Tanks: [
      'germ_jagdpanther',
      'germ_pzkpfw_v_ausf_a_panther',
      'germ_pzkpfw_v_ausf_g_panther',
      'germ_panzerjager_iv_l_70',
      'germ_panzerjager_iv_l_70_a',
      'germ_pzkpfw_iv_ausf_j',
      'germ_pzkpfw_vi_ausf_e_tiger',
      'germ_pzkpfw_vi_ausf_b_tiger_iip'
    ],
  },
  {
    'Category': 'weapons.category.tunisia.allies',
    Tanks: [
      'uk_a13_mk_2_1938',
      'uk_a13_mk_2_1939',
      'uk_crusader_mk_3',
      'uk_daimler_mk_2',
      'ussr_matilda_iii',
      'uk_mk_1_grant',
      'uk_sherman_ii'
    ]
  },
  {
    'Category': 'weapons.category.tunisia.axis',
    Tanks: [
      'it_m41_75_32',
      'it_ab_41',
      'it_m13_40_serie_3',
      'it_m14_41',
      'germ_pzkpfw_iii_ausf_j',
      'germ_pzkpfw_iii_ausf_n',
      'germ_pzkpfw_iv_ausf_f2'
    ],
  },
  {
    'Category': 'weapons.category.stalingrad.allies',
    Tanks: [
      'ussr_ht_130',
      'ussr_kv_1_l_11',
      'ussr_t_34_1940_l_11',
      'ussr_t_34_1941',
      'ussr_t_34e_stz',
      'ussr_t_70'
    ]
  },
  {
    'Category': 'weapons.category.stalingrad.axis',
    Tanks: [
      'germ_pzkpfw_ii_flamm',
      'germ_pzkpfw_iii_ausf_j',
      'germ_pzkpfw_iii_ausf_n',
      'germ_pzkpfw_iv_ausf_f2',
      'germ_pzkpfw_iv_ausf_g',
      'germ_stug_iii_ausf_f'
    ],
  },
  {
    'Category': 'weapons.category.pacific.allies',
    Tanks: [
      'us_lvt_a_1',
      'us_halftrack_m13',
      'us_m2a4',
      'us_m3a1_stuart',
      'us_m3a1_stuart_flame'
    ]
  },
  {
    'Category': 'weapons.category.pacific.axis',
    Tanks: [
      'jp_type_97_kai',
      'jp_type_95_ha_go',
      'jp_type_3_ka_chi',
      'jp_type_2_ka_mi',
      'jp_type_98_ke_ni',
      'jp_type_98_ta_se'
    ],
  }
];

/*
const STARUPGRADES = {
  tank: [
    {},
    { brake: 1.15, suspension: 1.15 },
    { engine: 1.15 },
    { dev: 0.85, horizSpeed: 1.15, vertSpeed: 1.2 },
  ],
};
*/
